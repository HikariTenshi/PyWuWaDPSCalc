...
function runCalculations() {
    ...
    for (let i = allBuffs.length - 1; i >= 0; i--) {
        let buff = allBuffs[i];
        if (((buff.triggeredBy === 'Passive' && buff.type === 'Buff') || buff.duration === 'Passive') && buff.specialCondition == null) {
            buff.duration = 9999;
        ...
        allBuffs.forEach(buff => {
            ...
            if (buff.specialCondition && !buff.specialCondition.includes('OnCast') && (buff.canActivate === 'Team' || buff.canActivate === activeCharacter)) { //special conditional
                ...
                isActivated = triggeredByConditions.some(function (condition) {
                    ...
                    if (condition.includes("Buff:")) { //check for the existence of a buff
                        ...
                        return (buffNamesString.includes(buffName) || buffNamesStringTeam.includes(buffName)) && extraCondition;
                        ...
                        passiveDamageQueue.forEach(passiveDamageQueued => {
                            ...
                            if (passiveDamageQueued != null && (condition.includes(passiveDamageQueued.name) || (condition === 'Passive' && passiveDamageQueued.limit != 1 && extraCondition && (passiveDamageQueued.type != 'TickOverTime' && buff.canActivate != 'Active')
                        ...
                        if (condition === 'Swap' && !skillRef.name.includes('Intro') && (skillRef.castTime == 0 || skillRef.name.includes('(Swap)'))) { //this is a swap-out skill
                            ...
                            return currentSkill.includes(condition) && applicationCheck && (buff.canActivate === activeCharacter || buff.canActivate === 'Team' || (skillRef.source === activeCharacter && introOutro));
                        }
                    } else {
            ...
            if (isActivated) { //activate this effect
                ...
                if (buff.type === 'ConsumeBuff') {
                    ...
                    console.log("adding a new type of passive damage " + buff.buffType);
                    ...
                    if (buff.type === "BuffEnergy" && currentTime >= buff.availableIn) { //add energy instead of adding the buff
                        ...
                        charData[activeCharacter].dCond.set(buff.buffType, charData[activeCharacter].dCond.get(buff.buffType) + buff.amount);
                    } else {
    ...
    if (CHECK_STATS) {
        statCheckMap.forEach((value, stat) => {
            if (totalDamage > 0) {
                ...
                var defense = (charData[activeCharacter].defense) * (1 + totalBuffMap.get('Defense') + bonusStats[activeCharacter].defense) + totalBuffMap.get('Flat Defense');
                var critMultiplier = (1 - Math.min(1, (charData[activeCharacter].crit + totalBuffMap.get('Crit')))) * 1 + Math.min(1, (charData[activeCharacter].crit + totalBuffMap.get('Crit'))) * (charData[activeCharacter].critDmg + totalBuffMap.get('Crit Dmg'));
                var damageMultiplier = getDamageMultiplier(classifications, totalBuffMap);
...
function rowToCharacterInfo(row, levelCap) {
    ...
    if (characterName === 'Jinhsi' && row[2] >= 2) {
        jinhsiS2 = true;
    }
    switch (build) {
        case "43311 (ER/ER)":
...
class PassiveDamage {
        ...
        if (procs > 0) {
            this.proccableBuffs.forEach(buff => {
                ...
                if (buffObject.type === 'StackingBuff') {
                    var stacksToAdd = 1;
                    var stackMult = 1 + (buffObject.triggeredBy === 'Passive' && buffObject.name.startsWith('Incandescence') ? 1 : 0);
                    ...
                    console.log("stacking buff name is procced: " + buffObject.name + "; " + buffObject.triggeredBy + "; stacks: " + buff.stacks + "; toAdd: " + stacksToAdd + "; mult: " + stackMult + "; target stacks: " + (Math.min((stacksToAdd * stackMult), buffObject.stackLimit)) + "; interval: " + effectiveInterval);
    ...
    calculateProc(activeCharacter) {
        ...
        var attack = (charData[this.owner].attack + weaponData[this.owner].attack) * (1 + totalBuffMap.get('Attack') + bonusStats[this.owner].attack + bonusAttack) + totalBuffMap.get('Flat Attack');
        var health = (charData[this.owner].health) * (1 + totalBuffMap.get('Health') + bonusStats[this.owner].health) + totalBuffMap.get('Flat Health');
        var defense = (charData[this.owner].defense) * (1 + totalBuffMap.get('Defense') + bonusStats[this.owner].defense) + totalBuffMap.get('Flat Defense');
        ...
        var damageMultiplier = getDamageMultiplier(this.classifications, totalBuffMap);

        var additiveValueKey = `${this.name} (Additive)`;
        var rawDamage = this.damage * (this.name.startsWith("Jué") ? 1 : skillLevelMultiplier) + (totalBuffMap.has(additiveValueKey) ? totalBuffMap.get(additiveValueKey) : 0);
        ...
        var scaleFactor = this.classifications.includes('Df') ? defense : (this.classifications.includes('Hp') ? health : attack);
        var totalDamage = rawDamage * scaleFactor * critMultiplier * damageMultiplier * (weaponData[this.owner].weapon.name === 'Nullify Damage' ? 0 : 1);
        ...
        updateDamage(this.name, this.classifications, this.owner, rawDamage * this.procMultiplier, totalDamage * this.procMultiplier, totalBuffMap)
        this.procMultiplier = 1;
        return totalDamage;
    }
    ...
    getNote() {
        var additiveValueKey = `${this.name} (Additive)`;
        if (this.limit == 1) {
            if (this.name === 'Star Glamour' && jinhsiS2)
...
function getDamageMultiplier(classification, totalBuffMap) {
    ...
    //console.log(`damage multiplier: {BONUS} ${damageBonus} * {MULTIPLIER} (1 + ${totalBuffMap.get('Multiplier')}) * {DEEPEN} (1 + ${damageDeepen}) * {RES} ${resMultiplier}`)
